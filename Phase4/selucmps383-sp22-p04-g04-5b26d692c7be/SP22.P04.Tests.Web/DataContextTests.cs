...
namespace SP22.P04.Tests.Web
{
    ...
    public class DataContextTests
    {
        ...
        [TestMethod]
        public void DataContext_HasSaleEvent()
        {
            using var scope = context.GetServices().CreateScope();
            var dbContext = GetDataContext(scope);
            if (dbContext == null)
            ...
            EnsureSet("SaleEvent", dbContext);
        }

        [TestMethod]
        public void DataContext_HasSaleEventProduct()
        {
            using var scope = context.GetServices().CreateScope();
            var dbContext = GetDataContext(scope);
            if (dbContext == null)
            ...
            EnsureSet("SaleEventProduct", dbContext);
        }
